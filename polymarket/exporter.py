...
class DataExporter:
    ...
    def _build_decision_tracker_report(self, ts: str) -> str:
        rows    = self.db.all_signals_with_resolution(hours=48, limit=500) if self.db else []
        summary = self.db.signal_summary() if self.db else {}
        ...
        lines = [
            "# Decision Tracker",
            f"\n> **Updated:** `{ts}` &nbsp;|&nbsp; "
            "All decisions with market outcome — refreshed every 5 minutes\n",
            "## Summary",
            _row(["Total Signals", "Resolved", "Wins", "Losses", "Pending", "Win Rate"]),
            _row(["-"*12, "-"*8, "-"*4, "-"*6, "-"*7, "-"*8]),
            _row([str(total), str(resolved), str(wins), str(losses), str(pending), win_rate]),
            "",
            "## Decision Log",
            "> Each signal: predicted direction vs actual market outcome (UP/DOWN winner)\n",
            _row(["Time (ET)", "Symbol", "Slug", "Predicted", "Trigger",
                  "Confidence", "P(UP)", "Bucket", "Elapsed%",
                  "Resolved (ET)", "Winner", "Result"]),
            _row(["-"*15, "-"*6, "-"*17, "-"*9, "-"*20,
                  "-"*10, "-"*5, "-"*14, "-"*8,
                  "-"*17, "-"*6, "-"*10]),
        ]
        ...
        for r in rows:
            slug        = _SLUG_FOR.get(r["symbol"], r["symbol"])
            bucket      = f"{r['vol_bucket']}+{r['trend_bucket']}" if r["vol_bucket"] else "—"
            res_time    = _to_et(r["resolved_at"]) if r["resolved_at"] else "—"
            winner      = r["market_winner"] or "—"
            sig_result  = r["signal_result"] or "pending"
            result_icon = _RESULT_ICON.get(sig_result, "⏳ Pending")
        ...
        if not rows:
            lines.append("> ℹ️ No decisions recorded yet.")
    ...
    # ── Report 4b: Trigger Performance Summary v2 (and versioned variants) ───────
    ...
    def _build_versioned_trigger_report(
        self,
        ts: str,
        title: str,
        description: str,
        ...
        Renders a per-symbol table with all canonical triggers, pending count, P&L.
        """
        if not self.db:
            return f"# {title}\n\nNo data available.\n"

        rows     = self.db.trigger_stats_for_strategy(start_ts, strategy)
        start_et = _to_et(start_ts)
    ...
    def _build_trigger_summary_v2_report(self, ts: str) -> str:
        ...
        return self._build_versioned_trigger_report(
            ts          = ts,
            title       = "Trigger Performance Summary v2",
            description = (
            ...
            strategy    = "v1",   # v2 report shows all V1 trades (legacy + tagged)
        )

    def _build_trigger_summary_v1_prod_report(self, ts: str) -> str:
        """V1.0 Production — ring-fenced trigger summary."""
        if not self.db:
            return "# Trigger Performance Summary — V1.0 Production\n\nNo data available.\n"
        return self._build_versioned_trigger_report(
            ts          = ts,
            title       = "Trigger Performance Summary — V1.0 Production",
            description = (
                "**V1.0 Production strategy** — forced_coin, forced_edge, pre_open, arb, "
                "directional_90pct (HighVol), trend_follow (HighVol+Trend). "
                "P&L is net profit/loss. Pending = not yet resolved."
            ),
            start_ts    = self.db.get_stats_start_ts_v1_prod(),
            strategy    = "v1",
        )

    def _build_trigger_summary_v3_report(self, ts: str) -> str:
        """V2.0 Dev — trend_follow + directional_90pct focus (forced_coin/edge suppressed)."""
        if not self.db:
            return "# Trigger Performance Summary — V2.0 Dev\n\nNo data available.\n"
        return self._build_versioned_trigger_report(
            ts          = ts,
            title       = "Trigger Performance Summary — V2.0 Dev",
            description = (
    ...
    def _build_trigger_summary_v4_report(self, ts: str) -> str:
        ...
        if not self.db:
            return "# Trigger Performance Summary — V3.0 Dev\n\nNo data available.\n"
        return self._build_versioned_trigger_report(
            ts          = ts,
            title       = "Trigger Performance Summary — V3.0 Dev",
            description = (
    ...
    def _build_market_v1_pnl_report(self, ts: str) -> str:
        """Market P&L summary filtered to V1.0 production strategy trades."""
        if not self.db:
            return "# Market P&L Summary — V1.0 Production\n\nNo data available.\n"

        rows = self.db.market_pnl_summary_for_strategy(strategy="v1")
        ...
        lines = [
            ...
            f"\n> **Updated:** `{ts}`\n",
            "> **V1.0 Production strategy** trades only. "
            "One row per 15-minute market window. "
            "P&L shown only for resolved markets.\n",
        ]
        ...
        if grand_bets == 0:
            lines.append("> ℹ️ No V1.0 trades recorded yet.")
        else:
            grand_wr  = f"{grand_wins/(grand_wins+grand_losses)*100:.0f}%" if (grand_wins + grand_losses) > 0 else "—"
            grand_pnl_s = f"+{grand_pnl:.2f}" if grand_pnl >= 0 else f"{grand_pnl:.2f}"
            lines += [
    ...
    def _build_readme(self, ts: str, markets: dict, trades: list, portfolio: dict) -> str:
        ...
        lines += [
            ...
            _row(["[Decision Summary](reports/decision_summary.md)",
                  "Analysis behind every signal (last 200)"]),
            ...
            _row(["[**V1.0 Prod** Market P&L](reports/market_V1_pnl.md)",
                  "V1.0 Production — market P&L view"]),
            "",
            "---",
            "_Auto-generated by **Bob the builder**_",
        ]
